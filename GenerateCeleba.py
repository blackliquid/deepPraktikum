...
sess = tf.Session()
saver = tf.train.import_meta_graph('modelCeleba.meta')
...
img = np.reshape(result, [50, 176, 176,3])

for i in range(0, batch_size_now):
    orig_img = np.reshape(batch[i], [176, 176, 3])
    scipy.misc.imsave('./imagesCeleba/orig_%s.png'%i, orig_img)
    generated_img = img[i]
    scipy.misc.imsave('./imagesCeleba/generated_%s.png'%i, generated_img)
